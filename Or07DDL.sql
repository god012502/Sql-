...
파일명 : Or07DDL.sql
DDL
Data Dfinition Language(데이터 정의어)
테이블, 뷰와 같은 객체를 생성 및 변경하는 쿼리문을 말한다.
*/
--system 계정으로 접속한 후 아래 명령을 실행
--계정 생성 시 c##을 생성하고 만들 수 있도록 session을 변경
...
-------------------------------------------------------------------------------------------
--education 계정으로 접속 한 후 학습을 진행합니다.
...
insert into tb_member values(1,'kosmo1','1234',
        '코스모155기','kosmo155@naver.com');       

insert into tb_member values(2,'kosmo2','1234',
        '코스모156기','kosmo155@naver.com');    

insert into tb_member values(3,'kosmo3','1234',
        '코스모157기','kosmo155@naver.com');
-- 삽입된 레코드 확인하기
...
as 
select*from tb_member where 1=0;
--테이블 구조만 복사되었으므로 레코드는 인출되지 않는다.
select*from tb_member_copy1;

--테이블복사 2 : 테이블의 스카마(구조)와 레코드까지 모두 복사
