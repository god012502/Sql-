...
제약조건 
    테이블 생성시 필요한 여러가지 제약조건에 대해 학습한다.
*/
...
/*
    제약조건 및 테이블목록 확인
        ...
        user_cons_colmns : 테이블에 지정된 제약 조건명과 컬럼명의 간략한 정보를 저장
        user_constraints : 테이블에 지정된 제약조건의 상세한 정보를 저장한다.
    ※이와 같이 제약조건이나 뷰, 프로시저 등의 정보를 
        저장하고 있는 시스템 테이블을 "데이터 사전"이라고 한다.
*/
...
insert into tb_primary1 (idx,user_id, user_name)
    values(3,'white', '화이트');
--PK로 지정된 컬럼에는 null을 입력할 수 없다. 에러발생됨    
insert into tb_primary1 (idx,user_id, user_name)
    values('','black', '블랙');    
--입력된 레코드 확인     
...
/*
PK 지정시 my_pk1로 제약명을 지정했으므로 에러 발생 시 해당
이름이 콘솔에 출력된다.
*/
insert into tb_primary2 values(2,'kosmo','코스모Error');

/*
형식 3] 테이블을 생성한 후 alter 문으로 제약조건 추가
...
desc tb_primary3;
/*
이미 생성된 테이블에 제약조건을 부여할 때는 alter를 사용하면 된다.
...
--데이터 사전에서 제약조건 확인하기
select * from user_constraints;
--제약 조건은 테이블을 대상으로 하므로 테이블 삭제 시 함께 삭제된다.
drop table tb_primary3;
--확인해보면 휴지통에 들어간 것을 볼수 있다.
select * from user_cons_columns;
--PK는 테이블당 하나만 생성 할 수 있다. 따라서 에러가 발생한다.
...
insert into tb_unique(idx, name, telephone, nickname)
    values(3, '슬기','','');
--unique는 중복을 허용하지 않지만 null은 여러 개 입력할수 있다
...
insert into tb_unique values(6,'장비','010-4444-4444','촉');
...
/*
    ...
    6번의 경우에는 동일한 데이터로 인식되어 에러가 발생된다.
*/
...
/*
    ...
    (ex. 회원제 게시판에서 글을 작성해 놓은 상태라면 그 회원은
        삭제할 수 없다. 참조 무결성 에러가 발생한다.)
...
create table tb_foreign1(
    ...
    f_name varchar2(50),
    /*
        자식테이블인 tb_foreign1에서 부모테이블인 tb_primary2의 user)id컬럼을
        참조하는 외래키를 생성한다.
    */
...
/*
    ...
    자식테이블에 부모를 참조하는 레코드가 남아있으면 
    부모테이블의 레코드를 delete 할 수 없다.
*/
...
/*
    형식 3] 테이블 생성 후 alter문으로 외래키 제약조건 추가
        alter table 테이블명 add [constraint 제약명]
            foreign key (컬럼명)
...
insert into tb_foreing4 values(1,'스팸1','stu1');
insert into tb_foreing4 values(2,'스팸2','stu1');
insert into tb_foreing4 values(3,'스팸3','stu1');
insert into tb_foreing4 values(4,'스팸4','stu1');
insert into tb_foreing4 values(5,'스팸5','stu1');
...
/*
    부모테이블에서 레코드를 삭제 할 경우 on delete cascade 옵션에 의해
    자식쪽까지 모든 레코드가 삭제된다.
    만약 해당 옵션을 부여하지 않았다면 레코드는 삭제되지 않고 오류가 
    발생하게 된다.
*/
...
insert into tb_foreing5 values(1,'스팸1','stu1');
insert into tb_foreing5 values(2,'스팸2','stu1');
insert into tb_foreing5 values(3,'스팸3','stu1');
insert into tb_foreing5 values(4,'스팸4','stu1');
insert into tb_foreing5 values(5,'스팸5','stu1');
...
/*
    삭제 시 부모테이블에서는 레코드가 삭제되지만, 자식테이블에서는 삭제되지 않는다.
    단 참조컬럼의 값이 null로 변경되어, 더 이상 참조할 수가 없는 레코드가 된다.
*/
...
create table tb_not_null(
    ...
    pw varchar2(30) null, -- null허용. 일반적으로 이렇게 쓰지 않는다.
    name varchar2(40) --null허용. 이와 같이 선언한다.
);
...
insert into tb_not_null values(5,' ','1111','오길동');
/*
    insert 쿼리에서 컬럼을 명시하지 않으면 null이 입력된다.
    따라서 idx컬럼에는 null을 입력할 수 없으므로 에러가 발생된다.
*/
