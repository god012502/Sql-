...
SQL Developer에서 주석 사용하기
    블럭 단위 주석 : 자바와 동일
    라인 단위 주석 : --실행문장, 하이픈 2개의 연속해서 기술한다.
*/
...
--select문 : 테이블에 저장된 레코드를 조회하는 SQL문
 ...
 /*
    형식] select 컬럼1, 컬럼2.... 혹은
        ...
        order by 정렬한 컬럼 asc(오름차순), desc(내림차순);
...
*/
select employeee_id,first_name, last_name, email, department_id from employees;
...
--null과 숫자의 연산은 계산이 불가하여 null의 결과가 출력된다.

/*
    AS(알리아스)
    테이블 혹은 컬럼에 별칭(별명)을 부여할 때 사용
    내가 원하는 이름(영문, 한글)으로 변경한 후 결과를 인출할 수 있다.
    활용] 급여+성과급율 → salcomm과 같은 형태로 별칭을 부여함
*/

--별칭은 한글로 설정할 수 있다.
SELECT employee_id, FIRST_name, salary, salary+100 as "급여 100증가" from employees;

--하지만 별칭은 영문으로 기술하는 것을 권장한다.
select first_name, salary, commission_pct, salary+(salary*commission_pct) as
SalCom from employees;
...
따라서 아래 SQL문을 실행하면 에러가 발생하진 않지만
아무런 결과도 인출되지 않는다.
*/
select employee_id "사원번호", first_name "이름", last_name "성" from employees 
where first_name='william';
...
select * from employees where last_name='Smith' and salary='8000';
--문자형은 싱글쿼테이션을 생략할 수 없다. 에러가 발생한다.
select * from employees where last_name=Smith and salary=8000;


/*
비교연산자를 통한 쿼리문 작성
이상, 이하와 같은 조건에 >, <=와 같은 비교 연산자를 사용할 수 있다.
...
--방법 1 : or를 사용. 이 경우 컬럼명을 반복적으로 사용해야 하므로 불편하다.
SELECT * FROM employees where salary=4200 or salary=6400 or salary=8000;
--방법 2 : in을 사용하면 컬럼명을 한번만 기술하면 되므로 편리하다.
...
/*
like 연산자
특정 키워드를 통한 문자열을 검색할 때 사용
형식] 컬럼명 like '%검색어%'
    와일드카드 사용법
    %: 모든 문자 혹은 문자열을 대체
        Ex) D로 시작 하는 단어 : D% - Da, Dae, Daewoo
            Z로 끝나는 단어 : %z - aZ, abxZ
...
/*
시나리오] 사원정보 테이블에서 급여가 낮은 순서대로 높은 순서로 정렬되어 인출하시오
*/
...
select first_name, salary, email, phone_number from employees order by
salary;--오름차순 정렬의 경우 asc를 생략할 수 있다.

/*
시나리오] 부서번호를 내림차순으로 정렬한 후 해당 부서에서 낮은 급여를
        받는 직원이 먼저 출력되도로고 SQL문을 작성하시오.
        출력항목 : 사원번호, 이름, 성, 급여, 부서번호 */
SELECT employee_id, FIRST_name, LAST_name, salary, department_id from
employees ORDER by department_id desc, salary;

/*
is null 혹은 is not null
값이  null 이거나 null이 아닌 레코드 인출
컬럼 중 null 값을 허용하는 경우 값을 입력하지 않으면 null값이 입력되는데
