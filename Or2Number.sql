...
파일명 : Or2Number.sql
숫자(수학) 관련 함수
설명 : 숫자 데이터를 처리하기 위한 숫자 관련 함수를 학습
        테이블 생성이 number 타입으로 선언된 컬럼에 저장된 데이터를 대상으로 한다.
*/
--현재 접속한 계정에 생성되어 있는 테이블과 뷰의 목록을 보여준다.
select * from tab;
--물리적으로 존재하는 테이블링ㄴ 경우 정상적으로 레코드가 인출된다.
select * from employees;
--존재하지 않는 테이블이라면 쿼리 에러가 발생한다.
SELECT * FROM kosmo;
/*
Dual 테이블
...
/*
trunc()
소수점을 특정자리수에서 잘라낼 때 사용
형식 ] trunc(컬럼명 혹은 값, 소수점 이하 자리수)
        두번째 인수가 
        양수 : 주어진 숫자만큼 소수점을 표현
        없을 때 : 정수부만 표현, 즉 소수점 아래부분을 버림
        음수 : 정수부를 숫자만큼 잘라 나머지를 0으로 채움
        */
SELECT trunc(12345.12345,2) from dual;--12345.12출력
...
SELECT ceil(32.2) from dual; -- 모두 33인출

SELECT floor(32.8) from dual;
SELECT floor(32.2) from dual; -- 모두 32인출
