...
-- => 위 SQL문으로 concat() 함수로 변경하면, 반복해서 써야하므로 불편하다.
...
형식] lpad('문자열', '전체자리수', '채울문자')
     => 전체 자리수에서 문자열의 길이만큼 제외한 나머지 좌(우)측 부분을 주어진 문자로 채워준다.
*/
...
형식 ] trim([leading | trailing | both] 제거할 문자  from 컬럼)
        -leading : 좌측의 공백 제거
        -trailing : 우측
        -both : 양쪽에서 제거 (default 값)
- 양쪽 끝의 문자만 제거되고, 중간의 문자는 제거할 수 없다.
- '문자'만 제거할 수 었고, '문자열'은 제거할 수 없다
*/
...
SELECT trim('다' from '다람쥐가 나무를 탑니다')from dual;
...
이를 통해 *를 출력하면 첫 글자를 제외한 남은 길이만큼을 마스킹 처리할 수 있다.
*/
select 
    first_name,
...
만약, 공백으로 문자열을 대체한다면 해당 문자열을 삭제하는 결과가 된다.
형식] replace(컬럼명 or 문자열, '변경 대상의 문자', '변경할 문자')
        ※trim(), ltrim(), rtrim()의 기능을 replace() 하나로 대체할 수 있으므로
         trim()에 비해 replace()가 훨씬 더 사용빈도가 높다.
*/
--특정 문자열을 변경
select substr('good morning john','morning','evening') from dual;
--문자열 삭제
select substr('good morning john','morning','') from dual;
