...
형변환 함수 / 기타 함수
설명 : 자료형을 다른 타입으로 변환해야 할 때 사용하는 함수와 기타함수
*/

/*
sysdate 
현재날짜와 시간을 초 단위로 반환해준다.
주로 게시판이나 회원가입 시 날짜를 입력하기 위해 사용된다.
오라클의 기본 서식이 xx/xx/xx이므로 출력시 시간은 표시되지 않지만,
...
/*
날짜포맷
오라클은 대소문자를 구분하지 않으므로,
서식문자 역시 구분없이 사용할 수 있다.
...
이를 제외한 나머지 문자열은 ""dmfh anRdjwnjdi gksek
서식문자를 감싸는게 아니므로 작성에 주의해야 한다.
*/
...
/*
숫자에 세자리마다 콤마 표시하기
자기수가 확실히 보장된다면 0을 사용하고, 
자리수가 확실치 않은 경우에는 9를 사용한다.
대신 공백은 trim으로 제거하면 된다.
*/
select 12345,
    to_char(12345,'000,000'), 
    ltrim(to_char(12345,'999,999')),ltrim(to_char(12345,'999,999')),
...
/*
to_date()
    문자열 데이터를 날짜형식으로 변환해서 출력해본다.
    기본서식은 년/월/일 순으로 저장된다.
*/
...
/*
아래와 같이 날짜포맥이 년-월-일 이 아닌경우에는 오라클이 인식하지 못하므로
...
*/ 
select 
    to_char(to_date('2020-10-14''yyyy-mm-dd'),'15:30:21','hh24:mi:ss')
from dual;
...
/*
방법1 : 날짜형식의 문자열을 substr()로 날짜부분만 잘래낸 후 사용한다.
        주어진 문자열은 년-월-일 형식이므로 별도의 서식없이도 인식된다.
*/        
select 
...
select
    to_date('2021"년 "01"월 "01"일"', 'dy'), "요일(월)"
from dual;

--답
--날짜 형식을 알 수 없어 에러발생됨
select 
    to_date('2021"년 "01"월 "01"일", 'yyyy"년"mm"월"dd"일"') "1 날짜 인식"
    to_char(to_date('2021"년 "01"월 "01"일", 'yyyy"년"mm"월"dd"일"'),'day') " 2 요일 인식"
from dual;
...
select
    ...
    decode(department_id, --컬럼명지정
    30, 'Purchasing', -- 조건 start
    50, 'Shipping', 
    60, 'IT',
    90, 'Executive',
    100, 'Finance', --조건 end
    '부서명 확인 안됨' --default값으로 조건에 적용되지 않는 나머지
    ) as dep_name
from employees;
