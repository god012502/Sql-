...
그룹으로 묶어서 연산한 후 결과를 반환하는 함수 혹은 쿼리문 
*/
...
/*
dintinct
동일한 값이 있는 경우 중복된 레코드를 제거한 후 하나의
레코드만 가져와서 보여준다.
...
보여지는 것은 하나의 레코드이지만 다수의 레코드가 
하나의 그룹으로 묶여진 결과이므로 통계적인 값을 계산할 수 있다.
...
--count() 함수를 통해 인출된 행의 갯수는 아래와 같이 일반적은 select문으로 검증할 수 있다
...
/*
count() 함수를 사용할 때는 위 2가지 방법 모두 가능하지만 * 사용을 권장한다.
컴럼의 특성 혹은 데이터에 따른 방해를 받지 않으므로 실행속도가 빠르다.
*/
...
select
    ltrim(to_char(avg(salary),'$999,000.0')) as AvgSalary
    from employees where department_id=80;
    
    
/*
min()/max()
최소, 최대값을 찾을 때 사용하는 함수   
*/
--전체 사원 중 급여가 가장 적은 직원은 누구인가요?

/*
아래 쿼리문은 에러가 발생된다.
그룹함수는 일반컬럼에 바로 사용할 수 없다.
...
/*
group by 절
여러개의 레코드를 하나의 그룹으로 묶은 후 그룹화하여 결과를 반환하는 쿼리문
...
/*
having
물리적으로 존재하는 컬럼이 아닌 그룹함수를 통해  논리적으로 생성된 컬럼의
조건을 추가할 때 사용한다.
