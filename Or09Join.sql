...
파일명 : Or09Join.sql
테이블 조인
2개 이상의 테이블을 동시에 참조하여 데이터를 인출해야 할 때 사용하는 SQL문
*/
...
1] inner join(내부조인)
    가장 많이 사용하는 조인문
    테이블간에 연결조건을 모두 만족하는 레코드를 검색할 때 사용한다
    
    일반적으로 기본키(primary key)와 외래키(Foreign key)를 사용하여
    join하는 경우가 대부분이다.
    
    2개의 테이블에 동일한 이름의 컬럼이 존재해야 하고, 
    이 경우 '테이블명.컬럼명'과 같이 기술한다
...
*/
--1. 지역테이블을 통해 seattle이 위치한 레코드의 일련번호를 확인
...
표준방식에서 사용한 inner join과 on을 제거하고 
조인의 조건은 where 절에 표기하는 방식이다.
*/
...
/*
2] outer join(외부조인)
    outer join은 inner join과는  달리 두 테이블 간의 조인 조건이 정확히 
    일치하지 않아도 기준이 되는 테이블에서 레코드를 인출하는 방식이다.
    outer join을 사용할 때는 반드시 기준이 되는 테이블을 결정하고 
    쿼리문을 작성해야 한다.
...
select
    ...
    De.department_id, department_name, city
from employees Em
...
3] self jon(셀프조인)
    ...
    즉 자신의 테이블과 조인을 맺는 것이다.
    셀프조인에서는 별칭이 테이블을 구분하는 구분자의 역할을 하므로 매우 중요하다.
    형식] select 별칭1.컬럼, 별칭2.컬럼..
            from 테이블 A 별칭1, 테이블 A 별칭2
            where 별칭1.컬럼=별칭2.컬럼;
*/
...
--2. 사원 입장의 매니져아이디와 매니전 입장의 사원아이디를 조인한다.
--3. 각 입장에서 필요한 컬럼을 select 절에 명시해서 결과를 확인한다.
select
    empClerk.employee_id,
    empClerk.first_name, empClerk.last_name,
    empManager.employee_id,
    empManager.first_name, empClerk.last_name,
from employees empClerk, employees empManager
where empClerk.manager_id=empManager.empoyee_id;
...
select
    ...
    empManager.first_name ||' '|| empClerk.last_name "매니저 이름"
    

from employees empClerk, employees empManager
where empClerk.manager_id=empManager.empManager.empoyee_id;
...
selcect * from employees where hire_date>'07/05/24';
...
/*
    using 
    join문에서 주로 사용하는 on절을 대체할 수 있는 문장
    형식 ] on 테이블1.컬럼=
...
select
    first_name, last_name, email, departments.department_id, 
    department_name,jobs.job_id, job_title, city, state_province
...
/*
    ...
    컬럼인것을 전재로 작성되기 때문에 굳이 별칭을 사용할 필요가 없기 때문이다.
    즉 using은 테이블의 별칭 및 on절을 제거하여 보다 
    심플하게 join 쿼리문을 작성할 수 있게 해준다.
*/
...
--문제에서 주어진 지역정보로 지역 일련번호를 확인 (결과:1500)    
