...
select
    employees_id, first_name, last_name, job_id, salary
...
/*
    결과로 인출된 2개의 급여 중 하나만 만족하면 되므로 복수행 연산자 any를 
    이용해서 서브 쿼리문을 작성하면 된다.
...
/*
    테이블의 전체 레코드를 대상으로 하되 이름으로 정렬된 상태로 
    레코드를 인출하기 때문에 테이블을 대체할 수 있다.
